...
def model_socp(A, b, c, d, I, k, n, p):
    ...
    m2.add_constraints((sum(vars[j] * A[k][j] for j in range(n)) <= b[k] for k in range(len(A))),
...
def solve_model(m2, tau):
    ...
    try:
        ...
        best_bound = m2.get_solve_details().best_bound
    except:
    ...
    return variables_value, objective_value, best_bound, tau, gap_cplex
...
#data = pd.read_csv('multiplicative_instances/p=6/small_instance{}.csv'.format(1)
    ...
    variables_value, gamma, best_bound, tau, gap_cplex = solve_model(m2, tau)
    cplex_solution_time = time.time() - cplex_start_time
    return variables_value, cplex_solution_time, best_bound, gamma, gap_cplex
